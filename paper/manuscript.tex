...
Those three cells match the product stack available when the experiments were
first run and the primary contrast emerged (notebook and grid dated 15 January
2026). At that time, thinking-enabled Flash scoring was already available on
Gemini 3 Flash (\texttt{gemini-3-flash-preview}) with an explicit
\texttt{thinkingLevel} control, but a Gemini 3 Flash Lite direct-response
path was not yet available. The ordinary fast non-thinking Lite path was
still Gemini 2.5 Flash Lite (\texttt{gemini-flash-lite-latest}), so we used
that for the direct-response arm. For thinking we used Gemini 3 Flash at the
low end of the dial (\texttt{minimal} and \texttt{low}) so the contrast
stayed near ordinary deployment cost and latency, rather than max reasoning.
Higher thinking levels were not tested; the aim was whether any extra
thinking helped on this task, not to map the full dial.
...
changes generation (2.5 Flash Lite versus Gemini 3 Flash), because that is
what the stack offered for fast Lite non-thinking versus thinking-capable
Flash when the experiments began. That is a real confound for attributing the
gap to thinking alone. It also makes the result harder to dismiss as a
weak-model artifact, and more surprising: the thinking cells used the newer,
more capable generation, so any intelligence advantage favored the thinking
arm, yet that arm still did not close the gap to the older Lite
direct-response setup on this task. For auditability the archive stores the
API aliases used at call time.
...
(Section~\ref{sec:gridresults} and Section~\ref{sec:jar}). The headline direct-response versus thinking contrast confounds thinking with
model generation (Gemini 2.5 Flash Lite versus Gemini 3 Flash). That pairing
is what was available when the experiments were first run: thinking on Gemini
3 Flash, but no Gemini 3 Flash Lite for a matched-generation direct-response
arm (Section~\ref{sec:models}). The confound works against the
direct-response result rather than for it. Any intelligence gap between Gemini
2.5 and Gemini 3 favored the thinking arm, so it is especially surprising that
the newer generation with thinking still underperformed the older Lite
direct-response setup on held-out liking, speed, and structured-output
reliability. The clean isolation of thinking level is the minimal-versus-low
contrast within Gemini 3 Flash. We do not conclude that deeper thinking is generally better or
...
Flash Lite and two thinking levels on Gemini 3 Flash. When the experiments
were first run and the primary contrast emerged, Gemini 3 Flash was already
available for thinking-enabled scoring, but Gemini 3 Flash Lite was not, so
the matched fast Lite non-thinking path remained 2.5 Flash Lite. The headline
contrast therefore confounds thinking setting with generation and cannot
attribute the gap to thinking alone; only the minimal-versus-low contrast
within Gemini 3 Flash isolates thinking level. We chose those two thinking
levels as the low end of the product dial for a deployment-like cost and
latency regime; higher thinking levels were not tested. The confound does not
rescue a weak direct-response model. Because any capability gap between Gemini
2.5 and Gemini 3 favored the thinking arm, the clarity of the
direct-response advantage is especially surprising and more informative than
a same-generation comparison would have been. Within the thinking-capable
