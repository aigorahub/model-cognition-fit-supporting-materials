...
first run and the main pattern was discovered (notebook and grid dated 15
January 2026). At that time, thinking-enabled Flash scoring was already
available on Gemini 3 Flash (\texttt{gemini-3-flash-preview}) with an explicit
...
gap to thinking alone. It also makes the main pattern harder to dismiss as a
weak-model artifact: the thinking cells used the newer generation, and they
still did not close the gap to the older Lite direct-response setup on this
...
generation, and they still underperformed the older Lite direct-response setup
on held-out liking, speed, and structured-output reliability. The clean
isolation of thinking level is the minimal-versus-low contrast within Gemini 3
Flash. We do not conclude that deeper thinking is generally better or
...
were first run and the main pattern appeared, Gemini 3 Flash was already
...
rescue a weak direct-response model: if anything, giving the thinking path the
newer generation makes the failure of extra thinking on this task more
informative. Within the thinking-capable generation at six points and
temperature 0.7, low thinking improved prediction and structured-output
reliability relative to minimal thinking but increased runtime, and neither
closed the gap to the direct-response configuration. Whether some task exists
at which higher thinking levels would dominate is an open question.
